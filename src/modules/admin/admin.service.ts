/* eslint-disable @typescript-eslint/no-explicit-any */
import HttpStatus from "http-status";
import { UserModel } from "../user/user.model";
import { IUser } from "../user/user.interface";
import AppError from "../../errors/AppError";
import { loginRequestEmailTemplate } from "./admin.utils";
import { sendEmail } from "../../utils/sendEmail";
import QueryBuilder from "../../builder/QueryBuilder";
import { searchStudents, searchTeachers } from "./admin.const";
import { LegalModel, NewsModel } from "../News/news.model";
import { ILegal, INews } from "../News/news.interface";
import { sendImageToCloudinary } from "../../utils/sendImageToCloudinary";

const createTeacher = async (payload: IUser) => {
  const isUserExist = await UserModel.findOne({
    email: payload?.email,
  });
  if (isUserExist) {
    throw new AppError(HttpStatus.BAD_REQUEST, "The User already exists");
  }
  payload.licenseExpiresAt = new Date();
  payload.licenseExpiresAt.setFullYear(
    payload.licenseExpiresAt.getFullYear() + 1
  );
  payload.isLicenseAvailable = true;
  // payload.role = "teacher";
  // console.log(payload);
  const result = await UserModel.create(payload);
  // console.log(result);
  if (result) {
    await sendEmail(
      result.email,
      "Verify you credintials by this link",
      loginRequestEmailTemplate(
        result.name as string,
        result.email,
        payload.password
      )
    );

    return {
      message: "Please check you email. Please verify to complete signIn.",
    };
  } else {
    return { message: "Somethine went wrong!" };
  }
};

const getAllTeachers = async (query: Record<string, unknown>) => {
  const teachersQuery = new QueryBuilder(
    UserModel.find(
      { isDeleted: false, role: "teacher" },
      "-password -otp -expiresAt -isVerified -licenseExpiresAt -isLicenseAvailable -passwordChangedAt"
    ),
    query
  )
    .search(searchTeachers)
    .filter()
    .sort()
    .paginate()
    .fields();

  const meta = await teachersQuery.countTotal();
  const result = (await teachersQuery.modelQuery).map((teacher: any) => {
    const obj = teacher.toObject();
    delete obj.password;
    return obj;
  });

  return { meta, result };
};

const getAllStudents = async (query: Record<string, unknown>) => {
  const teachersQuery = new QueryBuilder(
    UserModel.find(
      { isDeleted: false, role: "teacher" },
      "-password -otp -expiresAt -isVerified -licenseExpiresAt -isLicenseAvailable -passwordChangedAt"
    ),
    query
  )
    .search(searchStudents)
    .filter()
    .sort()
    .paginate()
    .fields();

  const meta = await teachersQuery.countTotal();
  const result = (await teachersQuery.modelQuery).map((teacher: any) => {
    const obj = teacher.toObject();
    delete obj.password;
    return obj;
  });

  return { meta, result };
};

const getEachTeacher = async (id: string) => {
  const result = await UserModel.findById(id).select(
    "-password -otp -expiresAt -isVerified -licenseExpiresAt -isLicenseAvailable -passwordChangedAt"
  );
  if (!result) {
    throw new AppError(HttpStatus.NOT_FOUND, "Teacher not found");
  }
  return result;
};

const updateLicense = async (
  id: string,
  payload: { isLicenseAvailable: boolean }
) => {
  const isTeacherExist = await UserModel.findById(id);
  if (!isTeacherExist) {
    throw new AppError(HttpStatus.NOT_FOUND, "Teacher not found");
  }
  const currentDate = new Date();
  const result = await UserModel.findByIdAndUpdate(
    isTeacherExist._id,
    {
      licenseExpiresAt: currentDate.setFullYear(currentDate.getFullYear() + 1),
      isLicenseAvailable: payload.isLicenseAvailable,
    },
    { new: true }
  );
  return result;
};

// eslint-disable-next-line no-undef
const addNews = async (payload: INews, file: Express.Multer.File) => {
  try {
    const newses = await NewsModel.find();
    if (newses && newses.length === 5) {
      throw new AppError(
        HttpStatus.BAD_REQUEST,
        "You cannot add more than 5 news & updates"
      );
    }
    if (!payload.title) {
      throw new Error("Title is required");
    }

    let thumbnailUrl = "";

    // Upload thumbnail if provided
    if (file) {
      const uploadedImage = await sendImageToCloudinary(
        file.buffer,
        file.originalname,
        file.mimetype
      );
      thumbnailUrl = uploadedImage.secure_url;
    } else {
      throw new Error("Thumbnail image is required");
    }

    const newsData = {
      title: payload.title,
      thumbnail: thumbnailUrl,
      category: payload.category,
      readTime: payload.readTime,
      publishedDate: payload.publishedDate,
      description: payload.description,
    };

    const result = await NewsModel.create(newsData);

    return {
      success: true,
      message: "News added successfully",
      data: result,
    };
  } catch (error: any) {
    return {
      success: false,
      message: error.message || "Failed to add news",
    };
  }
};

const getAllNews = async (query: Record<string, unknown>) => {
  const teachersQuery = new QueryBuilder(NewsModel.find(), query)
    .filter()
    .sort()
    .paginate()
    .fields();

  const meta = await teachersQuery.countTotal();
  const result = await teachersQuery.modelQuery;

  return { meta, result };
};

const getANews = async (id?: string) => {
  if (id) {
    // If id is provided, fetch by id
    const result = await NewsModel.findById(id);
    return result;
  } else {
    // If no id, fetch the first document (sorted by creation date or _id)
    const result = await NewsModel.findOne().sort({ createdAt: 1 }); // or sort by _id: 1
    return result;
  }
};

const getAllLegal = async (query: Record<string, unknown>) => {
  const teachersQuery = new QueryBuilder(LegalModel.find(), query)
    .filter()
    .sort()
    .paginate()
    .fields();

  const meta = await teachersQuery.countTotal();
  const result = await teachersQuery.modelQuery;

  return { meta, result };
};

const addLegal = async (payload: ILegal) => {
  if (!payload.description) {
    throw new AppError(HttpStatus.NOT_FOUND, "Description not found");
  }

  const legal = await LegalModel.create(payload);
  return legal;
};

export const AdminServices = {
  createTeacher,
  getAllTeachers,
  getEachTeacher,
  updateLicense,
  getAllStudents,
  addNews,
  getAllNews,
  getANews,
  addLegal,
  getAllLegal,
};
